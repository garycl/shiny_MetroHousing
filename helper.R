...
library(data.table)
library(geosphere)
setwd('/Users/Gary/Dropbox/Projects/shiny_project/data/csv')
...
# Create ZCTA data
zcta = inner_join(df_zcta, nhgis_zcta, by = 'ID', all = T)
...
# Find MSA city center and calculate distance
zcta = zcta %>%
  group_by(MetroName, State) %>%
  ...
  distinct() %>%
  right_join(zcta, by = c('MetroName', 'State'))
p1 = matrix(c(zcta$Longitude, zcta$Latitude), ncol = 2)
p2 = matrix(c(zcta$CentralLon, zcta$CentralLat), ncol = 2)
zcta$DistCBD = distGeo(p1, p2) / 10 ^ 3
zcta$LogDistCBD = log(1 + zcta$DistCBD)

# Keep top MSA
zcta = cbsa %>%
  select(MetroName, State, PopRank) %>%
  distinct() %>%
  inner_join(zcta, by = c('MetroName', 'State'))
